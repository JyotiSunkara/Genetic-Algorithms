...
# parent_population_errors = np.empty((POPULATION_SIZE, 2))
# child_population_errors = np.empty((POPULATION_SIZE, 2))
...
def calculate_fitness(population):
    ...
    rem = np.argsort(pop_fit[:,-1])
    pop_fit = pop_fit[rem]
    return pop_fit

def create_mating_pool(population_fitness):
    rem = np.argsort(population_fitness[:,-1])
    population_fitness = population_fitness[rem]
...
def main():
    ...
    for generation in range(num_generations):   
        ...
        population = population_fitness[VECTOR_SIZE :]
        ...
        for i in range(POPULATION_SIZE):
            ...
            rowDict = {'Generation': generation, 'Vector': population[i], 'Fitness': fitness[i][2]}
